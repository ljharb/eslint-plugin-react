...
module.exports = {
  ...
  create: Components.detect((context, components, utils) => {
    const typeAliases = {};
    const exactWrappers = getExactPropWrapperFunctions(context);

    function getPropTypesErrorMessage() {
      const formattedWrappers = formatPropWrapperFunctions(exactWrappers);
    ...
    function isNonExactPropWrapperFunction(node) {
      return (
        ...
        && !isExactPropWrapperFunction(context, getText(context, node.callee))
      );
    }
    ...
    return {
      ...
      MemberExpression(node) {
        if (!isPropTypesDeclaration(node) || exactWrappers.size === 0) {
          return;
        }
        ...
        if (isNonEmptyObjectExpression(right)) {
          ...
          const propsDefinition = findVariableByName(context, node, identifier);
