  ...
  create: Components.detect((context, components, utils) => {
    ...
    function isSingleSuperCall(body) {
      return (
        ...
        && 'expression' in body[0]
        && isCallExpression(body[0].expression)
     ...
     * @returns {node is Identifier | RestElement} `true` if the node doesn't have any side effects.
     */
     ...
     * @see eslint no-useless-constructor rule
     * @param {Parameter} ctorParam - A node to check.
     * @param {CallExpressionArgument} superArg - A node to check.
     ...
     * @see eslint no-useless-constructor rule
     * @param {Parameter[]} ctorParams - The parameters of a constructor to check.
     * @param {CallExpressionArgument[]} superArgs - The arguments of `super()` to check.
     ...
     * @see eslint no-useless-constructor rule
     * @param {Statement[]} body - constructor body content.
     * @param {Parameter[]} ctorParams - The params to check against super call.
    ...
    function isRedundantSuperCall(body, ctorParams) {
      return (
        ...
        && 'expression' in body[0]
        && body[0].expression
        && 'arguments' in body[0].expression
        && (
    ...
    function hasOtherProperties(node) {
      ...
      return properties.some((property) => {
        ...
        const isPropTypes = name === 'propTypes' || (name === 'props' && 'typeAnnotation' in property && property.typeAnnotation);
        const contextTypes = name === 'contextTypes';
        const defaultProps = name === 'defaultProps';
        const isUselessConstructor = 'kind' in property
    ...
    function visitClass(node) {
      if (ignorePureComponents && isPureComponent(node, context)) {
    ...
    return {
      ...
      VariableDeclarator(node) {
        ...
        if (!node.id || node.id.type !== 'ObjectPattern' || !astUtil.isThisExpression(node.init)) {
          return;
        }
      ...
      MemberExpression(node) {
        if (!astUtil.isThisExpression(node.object)) {
      ...
      ReturnStatement(node) {
        ...
        while (scope) {
          blockNode = scope.block && 'parent' in scope.block && scope.block.parent;
          if (
            blockNode
        ...
        const isRender = blockNode
          ...
          && 'key' in blockNode
          && blockNode.key
          && typeof blockNode.key === 'object'
          && 'name' in blockNode.key
      ...
      'Program:exit'() {
        ...
        Object.values(list)
          .filter((component) => (
            ...
            && !component.hasSCU
            && (isES5Component(component.node, context) || isES6Component(component.node, context))
          ))
          .forEach((component) => {
