...
module.exports = {
  ...
  create(context) {
    ...
    /** @type {Record<string | number, { arrowFunc: Set<unknown>, bindCall: Set<unknown>, bindExpression: Set<unknown>, func: Set<unknown> }> */
    ...
    /**
     * @param {(FunctionDeclaration | Expression | JSXEmptyExpression) & import('eslint').Rule.NodeParentExtension} node
     * @returns {'bindCall' | 'arrowFunc' | 'bindExpression' | 'func' | null}
     */
    function getNodeViolationType(node) {
      ...
      if (!allowArrowFunctions && node.type === 'ArrowFunctionExpression') {
        return 'arrowFunc';
      }
      if (
        !allowFunctions
      ...
      if (!allowBind && 'type' in node && node.type === 'BindExpression') {
    ...
    /**
     * @param {Exclude<ReturnType<typeof getNodeViolationType>, null>} violationType
    ...
    /** @param {FunctionDeclaration | JSXAttribute | VariableDeclarator} node */
    ...
    return {
      ...
      /** @param {JSXAttribute} node */
      JSXAttribute(node) {
        const isRef = ignoreRefs && propName(node) === 'ref';
        if (isRef || !node.value || !('expression' in node.value) || !node.value.expression) {
          return;
        }
        const nodeIsDOMComponent = isDOMComponent(node.parent);
        if (ignoreDOMComponents && nodeIsDOMComponent) {
          return;
        }
