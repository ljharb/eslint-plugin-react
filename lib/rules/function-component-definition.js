...
const { getTypeArguments } = require('../util/props');
...
/** @typedef {{ type: unknown, template: typeof NAMED_FUNCTION_TEMPLATES[keyof typeof NAMED_FUNCTION_TEMPLATES] | typeof UNNAMED_FUNCTION_TEMPLATES[keyof typeof UNNAMED_FUNCTION_TEMPLATES], range: unknown }} FixerOptions */
/** @typedef {(FunctionDeclaration | ArrowFunctionExpression | FunctionExpression) & NodeParentExtension} ValidNode */

/**
 * @param {FixerOptions['template']} template
 * @param {{ typeAnnotation: unknown, typeParams: unknown, params: ReturnType<typeof getParams>, returnType: ReturnType<typeof getNodeText>, body: ReturnType<typeof getBody>, name: ReturnType<typeof getName>, varType: unknown }} parts
*/
function buildFunction(template, parts) {
  return Object.keys(parts).reduce(
    /** @param {keyof parts} key */ (acc, key) => acc.replace(`{${key}}`, () => parts[key] || ''),
    template,
  );
}

const NAMED_FUNCTION_TEMPLATES = /** @type {const} */ ({
...
const UNNAMED_FUNCTION_TEMPLATES = /** @type {const} */ ({
...
function hasOneUnconstrainedTypeParam(node) {
  ...
  return !!nodeTypeArguments
    && 'params' in nodeTypeArguments
    && !!nodeTypeArguments.params
    && nodeTypeArguments.params.length === 1
    && (
      !('constraint' in nodeTypeArguments.params[0])
...
/** @typedef {'function-declaration' | 'arrow-function' | 'function-expression'} ComponentKind */
...
module.exports = {
  ...
  create: Components.detect((context, components) => {
    ...
    const namedConfig = /** @type {ComponentKind[]} */ ([].concat(
      configuration.namedComponents || 'function-declaration',
    ));
    const unnamedConfig = /** @type {ComponentKind[]} */ ([].concat(
      configuration.unnamedComponents || 'function-expression',
    ));

    /** @param {ValidNode} node @param {FixerOptions} options */
    ...
    /**
     * @param {ValidNode} node
     * @param {{ messageId: keyof typeof messages, fixerOptions: FixerOptions }} options
     * */
    ...
    /** @param {ValidNode} node @param {ComponentKind} functionType */
    ...
    // Public
    // --------------------------------------------------------------------------
    /** @type {[ValidNode, ComponentKind][]} */
    const validatePairs = [];
    let hasES6OrJsx = false;
    return {
      FunctionDeclaration(node) {
        validatePairs.push(/** @type {const} */ ([node, 'function-declaration']));
      },
      ArrowFunctionExpression(node) {
        validatePairs.push(/** @type {const} */ ([node, 'arrow-function']));
      },
      FunctionExpression(node) {
        validatePairs.push(/** @type {const} */ ([node, 'function-expression']));
      },
