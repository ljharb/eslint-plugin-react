...
function isNonspaceJSXTextOrJSXCurly(node) {
  return (isJSXText(node) && 'raw' in node && !isOnlyWhitespace(node.raw)) || node.type === 'JSXExpressionContainer';
}
 ...
 */
/** @type {(node: ASTNode) => node is JSXElement} */
...
module.exports = {
  ...
  create(context) {
    ...
    /**
     * @param {JSXFragment | JSXElement} node
     * @returns {boolean}
     */
    function canFix(node) {
      // Not safe to fix fragments without a jsx parent.
      if (!('parent' in node) || !(node.parent.type === 'JSXElement' || node.parent.type === 'JSXFragment')) {
        // const a = <></>
    ...
    function getFix(node) {
      ...
      return /** @type {Fixer} */ function fix(fixer) {
        ...
        const childrenText = 'selfClosing' in opener && opener.selfClosing ? '' : getText(context).slice(opener.range[1], closer.range[0]);
