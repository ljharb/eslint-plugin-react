...
module.exports = {
  ...
  create(context) {
    ...
    function shouldCheckContextTypes(node) {
      if (checkContextTypes && isContextTypesDeclaration(node)) {
        return true;
      }
      return false;
    }
    ...
    function shouldCheckChildContextTypes(node) {
      if (checkChildContextTypes && isChildContextTypesDeclaration(node)) {
        return true;
      }
      return false;
    }
        ...
        declarations.forEach((declaration) => {
          ...
          if (isCallExpression(value)) {
            ...
            value.arguments.forEach((arg) => {
              const name = isMemberExpression(arg) ? arg.property.name : arg.name;
    ...
    function checkNode(node) {
      ...
      if (node.type === 'ObjectExpression') {
        ...
        if (
          isPropWrapperFunction(context, getText(context, node.callee))
            && innerNode
        ) {
          checkNode(innerNode);
        }
    ...
    return {
      ...
      CallExpression(node) {
        if (
          ...
          && !isPropTypesDeclaration(node.callee)
        ) {
          return;
        }
        ...
        if (
          ...
          && (
            ...
            || getPropertyName(node.callee) === 'shape'
          )
        ) {
      ...
      ObjectExpression(node) {
        node.properties.forEach((property) => {
          ...
          if (
            !isPropTypesDeclaration(property)
