...
const nonCallableTypes = ['any', 'array', 'object'];

function isNonCallablePropType(type) {
  return nonCallableTypes.includes(type);
}
...
module.exports = {
  ...
  create(context) {
    ...
    function checkProperty(declaration) {
    ...
    function checkProperties(declarations) {
      if (declarations) {
        declarations.forEach(checkProperty);
      }
    }
    ...
    return {
      ...
      MemberExpression(node) {
        if (
          ...
          if (node.object.name === 'PropTypes' && isNonCallablePropType(node.property.name) && isForbidden(node.property.name)) {
            // reportIfForbidden(node.property.name, node.parent, node.property.name);
          }
          return;
        }
