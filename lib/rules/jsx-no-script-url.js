...
const { getLinkComponents } = require('../util/linkComponents');
...
function hasJavaScriptProtocol({ value }) {
  return value && value.type === 'Literal' && isJavaScriptProtocol.test(value.value);
}
...
module.exports = {
  ...
  create(context) {
    ...
    const { includeFromSettings } = objectOption;

    const linkComponents = getLinkComponents(includeFromSettings ? context : {});
