...
/** @type {(node: FunctionDeclaration | ArrowFunctionExpression | FunctionExpression | ClassDeclaration | CallExpression) => boolean} */
...
function isReturnStatementOfHook(node, context) {
  ...
  const callExpression = getClosestMatchingParent(node, context, isCallExpression);
  return (
    callExpression
    && 'callee' in callExpression
...
function isComponentInRenderProp(node, context, propNamePattern) {
  ...
  if (
    ...
    && 'parent' in jsxExpressionContainer
 ...
 * @returns {string | boolean} Name of the component, if any
 */
function resolveComponentName(node) {
  const parentName = 'id' in node && node.id && 'name' in node.id && node.id.name;
...
module.exports = {
  ...
  create: Components.detect((context, components, utils) => {
    ...
    function isComponentInsideCreateElementsProp(node) {
      ...
      return (
        createElementParent
        && 'arguments' in createElementParent
        && createElementParent.arguments
        && createElementParent.arguments[1] === getClosestMatchingParent(node, context, astUtil.isObjectExpression)
      );
    }
     ...
     * { footer: () => <div /> }
     * ```
     * @param {FunctionDeclaration | ArrowFunctionExpression | FunctionExpression | ClassDeclaration | CallExpression} node The AST node being checked
