...
/** @param {Expression | Super | undefined | false} node @param {Context} context */
function isCreateCloneElement(node, context) {
  ...
  if (node.type === 'Identifier') {
    const variable = findVariableByName(context, node, node.name);
    if (variable && variable.type === 'ImportSpecifier') {
      return 'source' in variable.parent
        && 'value' in variable.parent.source
        && variable.parent.source.value === 'react';
    }
  }
...
module.exports = {
  ...
  create(context) {
    ...
    function getMapIndexParamName(node) {
      ...
      if (!isFunctionLikeExpression(callbackArg)) {
        return null;
      }
      ...
      return 'name' in params[indexParamPosition] ? params[indexParamPosition].name : null;
    }
    ...
    return {
      ...
      'CallExpression, OptionalCallExpression'(node) {
        if (isCreateCloneElement('callee' in node && node.callee, context) && node.arguments.length > 1) {
          // React.createElement
          ...
          props.properties.forEach((prop) => {
            if (!('key' in prop) || !prop.key || !('name' in prop.key) || prop.key.name !== 'key') {
              // { ...foo }
              // { foo: bar }
              return;
            }
