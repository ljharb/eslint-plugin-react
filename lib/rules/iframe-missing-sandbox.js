...
/** @type {(context: Context, node: JSXOpeningElement | CallExpression, attribute: unknown) => void} */
...
/** @type {(context: Context, node: JSXOpeningElement) => void} */
...
/** @type {(context: Context, node: CallExpression) => void} */
function checkProps(context, node) {
  let sandboxAttributeFound = false;
  if (node.arguments.length > 1) {
    const props = node.arguments[1];
    const sandboxProp = 'properties' in props
      && props.properties
      && props.properties.find((x) => x.type === 'Property' && 'name' in x.key && x.key.name === 'sandbox');
    if (sandboxProp) {
      sandboxAttributeFound = true;
      if (
        'value' in sandboxProp
        && sandboxProp.value
