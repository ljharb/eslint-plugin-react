...
const {
  getPropertyName: getASTPropertyName,
...
module.exports = {
  ...
  create: Components.detect((context, components) => {
     ...
     * @returns {number[]} The matching patterns indexes. Return [Infinity] if there is no match.
     */
    ...
    function getPropertyName(node) {
      ...
      return getASTPropertyName(node);
    }
     ...
     * @param {(ClassElement | ObjectLiteralElement)[]} properties Array containing all the properties.
     */
    function checkPropsOrder(properties) {
      const propertiesInfos = properties.map((node) => ({
        name: getPropertyName(node),
        getter: 'kind' in node && node.kind === 'get',
        setter: 'kind' in node && node.kind === 'set',
        staticVariable: 'static' in node
          && node.static
          && (node.type === 'ClassProperty' || node.type === 'PropertyDefinition')
          && (!node.value || !isFunctionLikeExpression(node.value)),
    ...
    return {
      'Program:exit'() {
        Object.values(components.list()).forEach((component) => {
          const properties = getComponentProperties(component.node);
