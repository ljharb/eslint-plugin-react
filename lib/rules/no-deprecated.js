...
const { getFromContext } = require('../util/pragma');
...
module.exports = {
  ...
  create(context) {
    ...
    /** @type {(node: ClassDeclaration | ClassExpression | MemberExpression | ImportDeclaration | ObjectExpression | VariableDeclarator, methodName: string, methodNode?: ASTNode) => void} */
    ...
    function getReactModuleName(node) {
      ...
      Object.entries(MODULES).some(([key, moduleNames]) => {
        if (
          !('init' in node)
          && node.init
          ...
          moduleName = moduleNames.find((name) => 'name' in node.init && name === node.init.name);
        }
        return moduleName;
      });
    ...
    function getLifeCycleMethods(node) {
      ...
      return properties.map((property) => ({
        ...
        node: getPropertyNameNode(property),
      }));
    }
    ...
    return {
      ...
      VariableDeclarator(node) {
        ...
        ('id' in node && 'properties' in node.id ? node.id.properties : undefined).filter((p) => p.type !== 'RestElement' && p.key).forEach((property) => {
          checkDeprecation(
            node,
