...
/** @type {(elementName: unknown) => elementName is keyof typeof VOID_DOM_ELEMENTS} */
function isVoidDOMElement(elementName) {
  return typeof elementName === 'string' && Object.hasOwn(VOID_DOM_ELEMENTS, elementName);
}
...
module.exports = {
  ...
  create: (context) => ({
    ...
    JSXElement(node) {
      const elementName = 'name' in node.openingElement.name && node.openingElement.name.name;
      ...
      const hasChildrenAttributeOrDanger = attributes.some((attribute) => {
        if (!('name' in attribute) || !attribute.name) {
          return false;
        }
    ...
    CallExpression(node) {
      if (!isMemberExpression(node.callee) && node.callee.type !== 'Identifier') {
        return;
      }
