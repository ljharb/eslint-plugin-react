...
/** @type {(rawStringValue: string) => boolean} */
...
/** @type {(rawStringValue: string) => string} */
...
/** @type {(raw: string, node: ASTNode) => boolean} */
...
/** @type {(child: ASTNode) => boolean} */
function containsWhitespaceExpression(child) {
  if (child.type === 'JSXExpressionContainer' && 'value' in child.expression) {
...
function isLiteralWithTrailingWhiteSpaces(node) {
  return node.type && node.type === 'Literal' && typeof node.value === 'string' && isStringWithTrailingWhiteSpaces(node.value);
}
...
module.exports = {
  ...
  create(context) {
     ...
     * @param {JSXExpressionContainer} JSXExpressionNode - The AST node with an unnecessary JSX expression
     */
    function reportUnnecessaryCurly(JSXExpressionNode) {
      report(context, messages.unnecessaryCurly, 'unnecessaryCurly', {
        ...
        fix(fixer) {
          ...
          } else if (JSXExpressionNode.parent.type === 'JSXAttribute') {
    ...
    /** @param {JSXElement | Literal | JSXText} literalNode */
    ...
    /** @type {(node: JSXExpressionContainer) => void} */
    function lintUnnecessaryCurly(JSXExpressionNode) {
      ...
      if (
        (expression.type === 'Literal' || expression.type === 'JSXText')
    ...
    /** @type {(node: JSXElement | Literal | JSXText, config: object) => boolean} */
    ...
    return {
      ...
      /** @param {JSXElement | Literal | JSXText} node */
