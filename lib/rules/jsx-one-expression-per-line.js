...
module.exports = {
  ...
  create(context) {
    ...
    function nodeDescriptor(n) {
      return 'openingElement' in n && n.openingElement ? n.openingElement.name.name : getText(context, n).replace(/\n/g, '');
    }

    /** @type {(node: JSXElement | JSXFragment) => void} */
    function handleJSX(node) {
      ...
      /** @type {{ [key: number]: (Literal | JSXChild)[] }} */
      ...
      Object.keys(fixDetailsByNode).forEach((key) => {
        ...
        const leadingSpaceString = leadingSpace ? '\n{\' \'}' : '';
        const trailingSpaceString = trailingSpace ? '{\' \'}\n' : '';
        const leadingNewLineString = leadingNewLine ? '\n' : '';
        const trailingNewLineString = trailingNewLine ? '\n' : '';

        const replaceText = `${leadingSpaceString}${leadingNewLineString}${replacedSource}${trailingNewLineString}${trailingSpaceString}`;
