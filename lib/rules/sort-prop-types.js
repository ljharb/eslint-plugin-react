...
const { isPropTypesDeclaration } = require('../util/props');
  ...
  create(context) {
    ...
    function checkNode(node) {
      ...
      if (node.type === 'ObjectExpression') {
        ...
        const propTypesObject = findVariableByName(context, node, node.name);
        if (propTypesObject && 'properties' in propTypesObject && propTypesObject.properties) {
          checkSorted(propTypesObject.properties);
        }
      } else if (isCallExpression(node)) {
        const innerNode = node.arguments && node.arguments[0];
        if (isPropWrapperFunction(context, node.callee.name) && innerNode) {
          checkNode(innerNode);
        }
      }
    }
    ...
    return {
      CallExpression(node) {
        ...
        if ('properties' in firstArg && firstArg.properties) {
          checkSorted(firstArg.properties);
        } else if (firstArg.type === 'Identifier') {
          const variable = findVariableByName(context, node, firstArg.name);
          if (variable && 'properties' in variable && variable.properties) {
