...
function isMultilined(node) {
  return !!node && 'loc' in node && node.loc.start.line !== node.loc.end.line;
}
...
module.exports = {
  ...
  create(context) {
    ...
    /** @param {ASTNode | undefined} element */
    function isNonBlockComment(element) {
      return !!element && !isBlockCommentInCurlyBraces(element) && (element.type === 'JSXElement' || element.type === 'JSXExpressionContainer');
    }

    return {
      'Program:exit'() {
        jsxElementParents.forEach((parent) => {
          parent.children.forEach(/** @type {(element: ASTNode, index: number, elements: ASTNode[]) => void} */ ((element, index, elements) => {
      ...
      /** @param {JSXElement | JSXExpressionContainer} node */
