  ...
  create(context) {
    function isSetStateCall(node) {
      return isCallExpression(node)
        ...
        && astUtil.isThisExpression(node.callee.object);
    }
    ...
    return {
      ...
      MemberExpression(node) {
        if (
          ...
          && astUtil.isThisExpression(node.object)
      ...
      ObjectPattern(node) {
        const isDerivedFromThis = 'init' in node.parent && astUtil.isThisExpression(node.parent.init);
