...
/** @typedef {{ stateFields: Set<unknown>, usedStateFields: Set<unknown>, aliases: null | Set<unknown> }} ClassInfo */
...
function isSetStateCall(node) {
  const unwrappedCalleeNode = unwrapTSAsExpression(node.callee);

  return (
    isMemberExpression(unwrappedCalleeNode)
...
module.exports = {
  ...
  create(context) {
    ...
    function isStateReference(node) {
      ...
      const isDirectStateReference = isMemberExpression(node)
    ...
    // Takes an ObjectExpression node and adds all named Property nodes to the current set of state fields.
    ...
    /** @param {Pattern} left @param {Expression} right */
    function handleAssignment(left, right) {
      const unwrappedRight = unwrapTSAsExpression(right);
    ...
    /** @param {PropertyDefinition | ClassProperty} node */
    ...
    return {
      ...
      CallExpression(node) {
        ...
        const unwrappedArgumentNode = unwrapTSAsExpression(unwrappedNode.arguments[0]);
        ...
        if (
          ...
          const unwrappedBodyNode = unwrapTSAsExpression(unwrappedArgumentNode.body);
      ...
      'ClassProperty, PropertyDefinition'(node) {
        ...
        const unwrappedValueNode = unwrapTSAsExpression(node.value);
      ...
      'PropertyDefinition, ClassProperty'(node) {
        ...
        if (argVar) {
          ...
          stateRefs.forEach((ref) => {
            ...
            if (identifier && isMemberExpression(identifier.parent)) {
      ...
      FunctionExpression(node) {
        ...
        const { parent } = node;
        if (!isES5Component(parent.parent, context)) {
          return;
        }
      ...
      AssignmentExpression(node) {
        ...
        const unwrappedLeft = unwrapTSAsExpression(node.left);
        const unwrappedRight = unwrapTSAsExpression(node.right);

        // Check for assignments like `this.state = {}`
        if (
          isMemberExpression(unwrappedLeft)
      ...
      'MemberExpression, OptionalMemberExpression'(node) {
        ...
        if (isStateReference(unwrapTSAsExpression(node.object))) {
        ...
        } else if (isStateReference(node) && 'parent' in node && isCallExpression(node.parent)) {
          classInfo = null;
        }
      },
      ...
      /** @param {ExperimentalSpreadProperty | SpreadElement} node */
