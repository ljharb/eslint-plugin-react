...
module.exports = {
  ...
  create(context) {
    ...
    function hasJSX(node) {
      return isJSX(node) || (isExpression(node) && isJSX(node.expression));
    }
    ...
    function getDepth(node) {
      ...
      while (isJSX(node.parent) || isExpression(node.parent)) {
        node = node.parent;
        if (isJSX(node)) {
          count += 1;
        }
    ...
    function findJSXElementOrFragment(startNode, name, previousReferences) {
      function find(refs, prevRefs) {
        for (let i = refs.length - 1; i >= 0; i--) {
          if (typeof refs[i].writeExpr !== 'undefined') {
            ...
            return (isJSX(writeExpr) && writeExpr) || (
              writeExpr
      ...
      const variable = getVariableFromContext(context, startNode, name);
