...
const { getPropertyNameNode } = require('../util/ast');
const {
...
// Rule messages
// ------------------------------------------------------------------------------
const ERROR_MESSAGES = /** @type {const} */ ({
...
// Properties to check
// ------------------------------------------------------------------------------
const propertiesToCheck = /** @type {const} */ ({
  propTypes: isPropTypesDeclaration,
  defaultProps: isDefaultPropsDeclaration,
  childContextTypes: isChildContextTypesDeclaration,
  contextTypes: isContextTypesDeclaration,
  contextType: isContextTypeDeclaration,
  displayName: /** @param {ASTNode} node */ (node) => isDisplayNameDeclaration(getPropertyNameNode(node)),
});
  ...
  create: Components.detect((context, _components, utils) => {
    ...
    return {
      ...
      'ClassProperty, PropertyDefinition'(node) {
        if (!getParentES6Component(context, node)) {
          return;
        }
      ...
      MemberExpression(node) {
        ...
        if (!relatedComponent || !isES6Component(relatedComponent.node, context)) {
          return;
        }
