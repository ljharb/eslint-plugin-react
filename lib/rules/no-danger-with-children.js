...
const { isWhiteSpaces } = require('../util/jsx');
const docsUrl = require('../util/docsUrl');
const report = require('../util/report');

const isMemberExpression = require('../util/ast').isMemberExpression;
...
module.exports = {
  ...
  create(context) {
     ...
     * @param {CallExpressionArgument} node - ObjectExpression node
    ...
    function findObjectProp(node, propName, seenProps) {
      ...
      return node.properties.find((prop) => {
        ...
        if (prop.type === 'ExperimentalSpreadProperty' || prop.type === 'SpreadElement') {
          const variable = getVariableFromContext(context, node, prop.argument.name);
     ...
     * @param {JSXElement} node - JSXElement node
     * @param {string} propName - name of the prop to look for
     * @returns {object | boolean}
     */
    function findJsxProp(node, propName) {
      ...
      return attributes.find((attribute) => {
        if (attribute.type === 'JSXSpreadAttribute') {
          const variable = getVariableFromContext(context, node, attribute.argument.name);
          if (variable && variable.defs.length && variable.defs[0].node.init) {
            return findObjectProp(variable.defs[0].node.init, propName, []);
          }
        }

        return 'name' in attribute && attribute.name && attribute.name.name === propName;
      });
    }
    ...
    function isLineBreak(node) {
      ...
      const nodeIsWhiteSpaces = isWhiteSpaces(node.value);

      return isLiteral && isMultiline && nodeIsWhiteSpaces;
    }
      ...
      CallExpression(node) {
        if (
          ...
          if (props.type === 'Identifier') {
            const variable = getVariableFromContext(context, node, props.name);
            if (variable && variable.defs.length && variable.defs[0].node.init) {
