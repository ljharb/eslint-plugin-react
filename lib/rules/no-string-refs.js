...
const { isThisExpression } = require('../util/ast');
...
module.exports = {
  ...
  create(context) {
     ...
     * @param {MemberExpression} node The AST node being checked.
     * @returns {boolean} True if we are using refs, false if not.
     */
    function isRefsUsage(node) {
      return !!(
        (getParentES6Component(context, node) || getParentES5Component(context, node))
     ...
     * @param {JSXAttribute} node The AST node being checked.
