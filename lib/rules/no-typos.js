  ...
  create: Components.detect((context, components, utils) => {
    ...
    function checkValidCallExpression(node) {
      ...
      if (isMemberExpression(callee)) {
        ...
        } else if (isMemberExpression(callee) && callee.property.name === 'oneOfType') {
    ...
    return {
      ...
      /** @type {(node: PropertyDefinitionNode) => void} */
      'ClassProperty, PropertyDefinition'(node) {
        if (!node.static || !isES6Component(node.parent.parent, context)) {
          return;
        }
