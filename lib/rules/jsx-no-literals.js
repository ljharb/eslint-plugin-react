...
/** @type {NonNullable<import('json-schema').JSONSchema4['properties']>} */
...
module.exports = {
  ...
  create(context) {
     ...
     * @param {Literal | JSXAttribute | JSXText | TemplateLiteral} node
     * @returns {JSXElement[]}
     */
    ...
    function hasJSXElementParentOrGrandParent(node) {
      const { parent, grandParent } = getParentAndGrandParent(node);
      return some(iterFrom([parent, grandParent]), (parent) => jsxElementTypes.has(parent.type));
    }
    ...
    function isViableTextNode(node, resolvedConfig) {
      /** @type {Iterator<unknown>} */
      const textValues = iterFrom([trimIfString(node.raw), trimIfString(node.value)]);
      if (some(textValues, /** @type {(value: unknown) => boolean} */ (value) => typeof value === 'string' && resolvedConfig.allowedStrings.has(value))) {
        return false;
      }
    ...
    function getOverrideConfig(node) {
      ...
      for (let i = 0; i < allAncestorElements.length; i++) {
        const ancestorElement = allAncestorElements[i];
    ...
    return Object.assign(hasElementOverrides ? {
      ...
      TemplateLiteral(node) {
        const { parent, grandParent } = getParentAndGrandParent(node);
        const isParentJSXExpressionCont = parent.type === 'JSXExpressionContainer';
        const isParentJSXElement = grandParent.type === 'JSXElement';
