 ...
 * @param {ASTNode} node - JSXOpeningElement to check.
...
/** @type {(node: undefined | null | ASTNode) => node is JSXElement | JSXFragment} */
function isJSX(node) {
  return !!node
    && typeof node === 'object'
...
function isReturningJSX(context, ASTnode, strict, ignoreNull) {
  const isJSXValue = /** @type {(node: ASTNode) => boolean} */ (node) => {
    if (!node) {
      return false;
    }
    switch (node.type) {
    ...
    traverse(node, {
      enter(childNode) {
        ...
        switch (childNode.type) {
          ...
          case 'ConditionalExpression':
            if (
              'value' in childNode.consequent
              && childNode.consequent.value === null
              && 'value' in childNode.alternate
