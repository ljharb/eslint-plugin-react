...
function isPropTypesDeclaration(node) {
  if (node && (node.type === 'ClassProperty' || node.type === 'PropertyDefinition')) {
    // Flow support
    if (node.typeAnnotation && 'name' in node.key && node.key.name === 'props') {
      return true;
    }
  }
...
function isContextTypesDeclaration(node) {
  if (node && (node.type === 'ClassProperty' || node.type === 'PropertyDefinition')) {
    // Flow support
    if (node.typeAnnotation && 'name' in node.key && node.key.name === 'context') {
      return true;
    }
  }
...
function isDisplayNameDeclaration(node) {
  switch (node.type) {
    ...
    case 'PropertyDefinition':
      return node.key && 'name' in node.key && node.key.name === 'displayName';
    case 'Identifier':
...
function isRequiredPropType(propTypeExpression) {
  return isMemberExpression(propTypeExpression)
    && 'property' in propTypeExpression
    && 'name' in propTypeExpression.property
