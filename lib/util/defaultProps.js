...
const { isDefaultPropsDeclaration } = require('./props');
const { findVariableByName } = require('./variable');
...
module.exports = function defaultPropsInstructions(context, components, utils) {
  ...
  function resolveNodeValue(node) {
    if (node.type === 'Identifier') {
      return findVariableByName(context, node, node.name);
    }
    if (
   ...
   * @param   {ObjectExpression} objectExpression ObjectExpression node.
   * @returns {object | string} Object representation of a defaultProp, to be consumed by
   * `addDefaultPropsToComponent`, or string "unresolved", if the defaultProps
   * from this ObjectExpression can't be resolved.
   */
  function getDefaultPropsFromObjectExpression({ properties }) {
    const hasSpread = properties.some((property) => property.type === 'ExperimentalSpreadProperty' || property.type === 'SpreadElement');

    if (hasSpread) {
      return 'unresolved';
    }

    return properties.map((defaultProp) => ({
   ...
   * @param {import('./Components').Component} component The component to add the defaultProps to.
   * @param {object[] | 'unresolved'} defaultProps defaultProps to add to the component or the string "unresolved" if this component has defaultProps that can't be resolved.
   * @returns {void}
   */
  ...
  return {
    MemberExpression(node) {
      ...
      if (
        astUtil.isMemberExpression(node.parent)
        && node.parent.parent
    ...
    MethodDefinition(node) {
      ...
      const component = components.get(getParentES6Component(context, node));
      if (!component) {
        return;
      }
