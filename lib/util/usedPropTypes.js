...
function isThisDotProps(node) {
  return astUtil.isMemberExpression(node)
    && astUtil.isThisExpression(astUtil.unwrapTSAsExpression(node.object))
...
module.exports = function usedPropTypesInstructions(context, components, utils) {
    ...
    if (astUtil.isMemberExpression(node)) {
      ...
      if (name) {
        ...
        // Match props.foo.bar, don't match bar[props.foo]
          astUtil.isMemberExpression(parent)
          && 'object' in parent
          && parent.object === node
        ) {
  ...
  return {
    VariableDeclarator(node) {
      ...
      // let {props} = this
      if (astUtil.isThisExpression(unwrappedInitNode) && propsProperty && propsProperty.value.name === 'props') {
