...
module.exports = function isDestructuredFromPragmaImport(context, node, variable) {
  ...
  const variableInScope = getVariableFromContext(context, node, variable);
  if (variableInScope) {
    ...
    if (latestDef) {
      ...
      if (latestDef.node.type === 'VariableDeclarator' && latestDef.node.init) {
        ...
        if (
          isMemberExpression(latestDef.node.init)
        ...
        if (isCallExpression(latestDef.node.init)) {
        ...
        if (
          !requireExpression
          && isMemberExpression(latestDef.node.init)
          && isCallExpression(latestDef.node.init.object)
        ) {
