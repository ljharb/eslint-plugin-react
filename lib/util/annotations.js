...
const { getFirstTokens } = require('./eslint');
...
function isAnnotatedFunctionPropsDeclaration(node, context) {
  if (!node || !('params' in node) || !node.params || !node.params.length) {
    return false;
  }
  ...
  const isAnnotated = 'typeAnnotation' in typeNode && !!typeNode.typeAnnotation;
