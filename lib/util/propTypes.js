...
const { testFlowVersion } = require('./version');
const { isPropWrapperFunction } = require('./propWrapper');
const {
  findReturnStatement,
  getKeyValue,
  isCallExpression,
 ...
 * @param {ASTNode} node
 * @returns {node is FunctionDeclaration | FunctionExpression | ArrowFunctionExpression}
 */
...
function isSuperTypeParameterPropsDeclaration(node) {
  if (node && (node.type === 'ClassDeclaration' || node.type === 'ClassExpression')) {
    const parameters = getSuperTypeArguments(node);
 ...
 * @param {Context} context Array of properties to iterate.
 * @param {ASTNode[]} properties Array of properties to iterate.
...
/** @type {(node: { parent: VariableDeclarator }) => boolean} */
...
module.exports = function propTypesInstructions(context, components, utils) {
  ...
  const { customValidators } = configuration;
  ...
  const typeDeclarationBuilders = {
    ...
    ObjectTypeAnnotation(annotation, parentName, seen) {
      ...
      const containsIndexers = 'indexers' in annotation
        && !!annotation.indexers
  ...
  function resolveTypeAnnotation(node) {
    let annotation = ('left' in node && node.left && 'typeAnnotation' in node.left && node.left.typeAnnotation)
      || ('typeAnnotation' in node && node.typeAnnotation)
      || node;
   ...
   * The representation is used to verify nested used properties.
   * @type {(node: EntityName) => string | undefined}
   */
  function getLeftMostTypeName(node) {
    if ('name' in node && node.name) return node.name;
    if ('left' in node && node.left) return getLeftMostTypeName(node.left);
  }

  /** @type {(node: EntityName) => string | undefined} */
  function getRightMostTypeName(node) {
    if ('name' in node && node.name) return node.name;
    if ('right' in node && node.right) return getRightMostTypeName(node.right);
  }

  /**
   * Returns true if the node is either a interface or type alias declaration
   * @type {(node: ASTNode) => node is TSInterfaceDeclaration | TSTypeAliasDeclaration}
   */
  function filterInterfaceOrTypeAlias(node) {
    return isTSInterfaceDeclaration(node) || isTSTypeAliasDeclaration(node);
  }
  ...
  class DeclarePropTypesForTSTypeAnnotation {
    ...
    searchDeclarationByName(node) {
      ...
      if (isTSTypeReference(node)) {
        ...
        const nodeTypeArguments = getTypeArguments(node);
        ...
        if (!node.expression && 'id' in node && node.id) {
       ...
       */
      const candidateTypes = this.sourceCode.ast.body.filter((item) => isTSTypeDeclaration(item));

      const declarations = candidateTypes.flatMap((type) => (
        'declarations' in type
        && (
          type.declarations
          || (
            type.declaration
      ...
      const typeDeclaration = declarations.filter((dec) => (
        dec
        && typeof dec === 'object'
        && 'id' in dec
        && typeof dec.id === 'object'
        && 'name' in dec.id
        && dec.id.name === typeName
      ));
        ...
        if (nodeTypeArguments.params.length === 1) {
          ...
          if (isTSTypeQuery(returnType)) {
            const returnTypeFunction = this.sourceCode.ast.body
              .filter((item) => item.type === 'VariableDeclaration' && item.declarations.find((dec) => dec.id.name === returnType.exprName.name))
                  ...
                  switch (res.type) {
                    case 'ObjectExpression':
                      iterateProperties(context, res.properties, (key, value, propNode) => {
                        if (propNode && isCallExpression(propNode.argument)) {
                          const propNodeTypeArguments = getTypeArguments(propNode.argument);
                      ...
                      break;
                    case 'CallExpression':
                      if (getTypeArguments(res)) {
                        this.visitTSNode(getTypeArguments(res));
                      } else {
                        // Ignore this CallExpression return value since it doesn't have any typeParameters to let us know it's types.
                        this.shouldIgnorePropTypes = true;
                      }
  ...
  function markPropTypesAsDeclared(node, propTypes, rootNode) {
    ...
    switch (propTypes && propTypes.type) {
        ...
        if (propTypes && propTypes.parent && propTypes.property) {
          ...
          while (n) {
            if (((n.type === 'AssignmentExpression') && isPropTypesDeclaration(n.left))
              || ((n.type === 'ClassProperty' || n.type === 'PropertyDefinition' || n.type === 'Property') && isPropTypesDeclaration(n))) {
      ...
      case 'Identifier': {
        const firstMatchingVariable = getVariableFromContext(context, node, propTypes.name);
      ...
      case 'GenericTypeAnnotation':
        if (propTypes.id.name === '$ReadOnly') {
          const propTypeArguments = getTypeArguments(propTypes);
  ...
  function markAnnotatedFunctionArgumentsAsDeclared(node, rootNode) {
    ...
    if (node.parent) {
      propTypesArguments = getTypeArguments(node.parent);
    }
    ...
    if (isNodeAnnotated) {
      ...
      if (
        ...
        && getTypeArguments(annotation) == null
      ) {
        return;
      }
