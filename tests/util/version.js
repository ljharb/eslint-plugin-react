...
describe('Version', () => {
  ...
  describe('Detect version', () => {
    ...
    it('matches detected version', () => {
      ...
      assert.equal(testReactVersion(context, '>= 1.2.3'), true);
      assert.equal(testReactVersion(context, '>= 1.2.4'), false);
      assert.equal(testFlowVersion(context, '>= 0.92.0'), true);
    });
    ...
    it('matches detected version in sibling project', () => {
      ...
      assert.equal(testReactVersion(context, '>= 2.3.4'), true);
      assert.equal(testReactVersion(context, '>= 2.3.5'), false);
      assert.equal(testFlowVersion(context, '>= 2.92.0'), true);
    });
    ...
    it('matches detected version in child project', () => {
      ...
      assert.equal(testReactVersion(context, '>= 3.4.5'), true);
      assert.equal(testReactVersion(context, '>= 3.4.6'), false);
      assert.equal(testFlowVersion(context, '>= 3.92.0'), true);
    });
    ...
    it('assumes latest version if react is not installed', () => {
      ...
      assert.equal(testReactVersion(context, '999.999.999'), true);
    ...
    it('uses default version from settings if provided and react is not installed', () => {
      ...
      assert.equal(testReactVersion(context, '16.14.0'), true);
    ...
    it('assumes latest version if flow-bin is not installed', () => {
      assert.equal(testFlowVersion(context, '999.999.999'), true);
  ...
  describe('string version', () => {
    ...
    it('works with react', () => {
      assert.equal(testReactVersion(context, '>= 0.14.0'), true);
      assert.equal(testReactVersion(context, '>= 15.0.0'), true);
      assert.equal(testReactVersion(context, '>= 16.0.0'), false);
    });

    it('works with flow', () => {
      assert.equal(testFlowVersion(context, '>= 1.1.0'), true);
      assert.equal(testFlowVersion(context, '>= 1.2.0'), true);
      assert.equal(testFlowVersion(context, '>= 1.3.0'), false);
    });

    it('fails nicely with an invalid react version', () => {
      assert.equal(testReactVersion(invalidContext, '>= 15.0'), true);
      expectedErrorArgs = [
    ...
    it('fails nicely with an invalid flow version', () => {
      assert.equal(testFlowVersion(invalidContext, '>= 1.0'), true);
      expectedErrorArgs = [
  ...
  describe('non-string version', () => {
    ...
    it('works with react', () => {
      assert.equal(testReactVersion(context, '>= 0.14.0'), true, '>= 0.14.0');
      assert.equal(testReactVersion(context, '>= 15.0.0'), true, '>= 15.0.0');
      assert.equal(testReactVersion(context, '>= 16.0.0'), false, '>= 16.0.0');
